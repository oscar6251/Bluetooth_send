...
public class BluetoothService {
    ...
    private static final int SEND_PER_NUM = 10;
    ...
    private void connectionFailed() {
        Message msg = mHandler.obtainMessage(BluetoothListActivity. MESSAGE_TOAST_FAIL);
        Bundle bundle = new Bundle();
        bundle.putString(BluetoothListActivity.TOAST,
                "客户端连接失败，转为服务端监听连接");
        msg.setData(bundle);
    ...
    /**
     * @method 连接中断时设置状态为监听，并发送消息
     */
    private void connectionLost() {

        Message msg = mHandler.obtainMessage
                (BluetoothListActivity.MESSAGE_TOAST_LOST);
        //在界面发送一个通知的结果
        Bundle bundle = new Bundle();
        bundle.putString(BluetoothListActivity.TOAST, "蓝牙连接中断");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    ...
    private synchronized void connected(BluetoothSocket socket, BluetoothDevice device) {
        ...
        //若连接蓝牙成功，传送蓝牙名字到主界面
        Message msg = mHandler.obtainMessage
                (BluetoothListActivity.MESSAGE_DEVICE_NAME);
        Bundle bundle = new Bundle();
        bundle.putString(BluetoothListActivity.DEVICE_NAME, device.getName());
        msg.setData(bundle);
        mHandler.sendMessage(msg);
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    while ((bytes = mInStream.read(buffer)) != -1) {
                        ...
                        for (int i = 0; i < bytes; i++) {
                            ...
                            byteCount++;
                            //每10个发送一次
                            if (byteCount == SEND_PER_NUM) {
                                byteCount = 0;
                                mHandler.obtainMessage(BluetoothListActivity.MESSAGE_BLUETOOTH_DATA,
                                        stringBuilder.toString()).sendToTarget();
                                stringBuilder = new StringBuilder();
                            }
                        }
                    }
